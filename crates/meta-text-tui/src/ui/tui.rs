...
use crate::tui::{write_prompt, SharedTuiInfo, TuiInput, TuiManager, UiOptions};
...
impl TuiFrontend {
    ...
    /// * `options` - The `[ui]` preferences to honour (colours, theme, mouse and
    ///   auto-scroll). Pass [`UiOptions::default`] for the shipped defaults.
    ///
    /// # Errors
    ///
    ...
    pub async fn start(
        ...
        let mut manager = TuiManager::new(app_name, options, sender.clone()).await?;
    ...
    pub async fn run(&mut self) -> Result<()> {
        ...
        if !self.full_screen {
            self.spawn_stdin_reader()
                .context("start the keyboard reader")?;
            write_prompt();
        }
    ...
    ///
    /// Like the plain REPL, the reader runs on a plain OS thread rather than
    /// through `tokio::io::stdin()`. Tokio's stdin blocks on the runtime's
    /// blocking pool, and dropping the runtime waits for that pool: a read with
    /// nothing to return would keep the process alive after `/quit` or Ctrl+C
    /// while stdin stays open. A thread the runtime does not join cannot block
    /// teardown; EOF or a closed queue still sends `Quit`.
    ///
    /// # Errors
    ///
    /// Returns the failure to create the thread — a resource limit, typically
    /// `EAGAIN` — rather than panicking: the fallback interface is unusable
    /// without the reader, so the caller reports the fault.
    fn spawn_stdin_reader(&self) -> std::io::Result<()> {
        ...
        std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                loop {
                    line.clear();
                    // Bind the read before matching so the stdin guard is released
                    // at the end of the statement rather than living across the arms.
