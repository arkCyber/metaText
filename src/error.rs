...
impl MetaTextError {
    ...
    ///
    /// ```rust
    /// use meta_text::error::MetaTextError;
    ///
    ...
    pub fn is_critical(&self) -> bool {
        matches!(
            self,
            MetaTextError::Resource { .. } | MetaTextError::Internal { .. }
        )
    }
    ...
    pub fn user_message(&self) -> &'static str {
        match self {
            ...
            MetaTextError::Cryptographic { .. } => {
                "Security error. Please restart the application."
            }
            MetaTextError::Database { .. } => {
                "Data storage error. Please check available disk space."
            }
            MetaTextError::UserInterface { .. } => {
                "Interface error. Please restart the application."
            }
            MetaTextError::Message { .. } => "Message processing error. Please try again.",
            MetaTextError::Authentication { .. } => {
                "Authentication failed. Please check your credentials."
            }
            MetaTextError::Resource { .. } => {
                "System resource error. Please restart the application."
            }
            MetaTextError::Validation { .. } => {
                "Invalid input. Please check your data and try again."
            }
            MetaTextError::Internal { .. } => "Internal error. Please restart the application.",
        }
    }
    ...
    pub fn log_error(&self) {
        ...
        match self {
            ...
            MetaTextError::Network { .. } | MetaTextError::Database { .. } => {
                error!(
                    "🌐 [{}] System error: {:?}",
                    timestamp.format("%Y-%m-%d %H:%M:%S"),
                    self
            ...
            MetaTextError::Cryptographic { .. } | MetaTextError::Authentication { .. } => {
                error!(
                    "🔐 [{}] Security error: {:?}",
                    timestamp.format("%Y-%m-%d %H:%M:%S"),
                    self
...
impl ErrorContext {
    ...
    ///
    /// ```rust
    /// use meta_text::error::ErrorContext;
    ///
